''' 
 CYK algorithm for Context Free Language [works in O(n^3)]
 Authors: Chenguang Zhu, Mikhail Dubov
...
X = [[[False]*4 for i in range(10)] for j in range(10)]
...
# return all the possible productions with nonterminals (in CNF)
def possibleProd():
    result = []
    for ntA in range(VarNum):
        for ntB in range(VarNum):
            for ntC in range(VarNum):
                result.append((ntA, ntB, ntC)) #tuple
    return result
...
def calcCYK(w):
    ...
    # 1. Basis: calculating X[i,i]
    for i in range(L):
        for nonterm in range(VarNum):
            if existProd(nonterm, w[i], -1):
                X[i][i][nonterm] = True
    
    # 2. Induction: filling the table.
    existingProd = filter(lambda (A,B,C) : existProd(A, B, C), possibleProd())
    ...
    for row in (reversedRange):
        for i in (range(row+1)):
            ...
            for k in (range(L)[i:j]):
                for (A, B, C) in existingProd: # existing productions of form A->BC
